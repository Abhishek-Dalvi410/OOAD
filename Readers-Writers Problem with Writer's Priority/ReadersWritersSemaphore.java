...
class Database {
	...
	Semaphore s1 = new Semaphore(1); // For translating synchronized methods
	Semaphore s2_r = new Semaphore(0); // Waiting readers
	Semaphore s2_w = new Semaphore(0); // Waiting writers
	...
	/*
	 * public synchronized void request_read() { while (w == 1 || ww > 0) // writers
	 * priority condition try { wr++; wait(); wr--; } catch (Exception e) { } r++; }
	 */
	...
	/*
	 * public synchronized void done_read() { r--; notifyAll(); }
	 */
	...
	/*
	 * public synchronized void request_write() { while (r > 0 || w == 1) { try {
	 * ww++; wait(); ww--; } catch (Exception e) {} } w = 1; }
	 */
	...
	/*
	 * public synchronized void done_write() { w = 0; notifyAll(); }
	 */
