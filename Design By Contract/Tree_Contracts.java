...
class Contract {
	static void Invariant(IContract ct) throws Exception {
		if (!ct.check())
			throw new Exception();
	}

	static void Requires(IContract ct) throws Exception {
		if (!ct.check())
			throw new Exception();
	}

	static void Ensures(IContract ct) throws Exception {
		if (!ct.check())
			throw new Exception();
	}
}
...
abstract class AbsTree {
	...
	public boolean delete(int n) throws Exception { // cannot delete last value in tree
		...
		Contract.Invariant(() -> ordered());
		Contract.Requires(() -> (this.max().value != this.min().value));
...
class DupTree extends AbsTree {
	...
	public boolean insert(int n) throws Exception {
		...
		Contract.Ensures(() -> (member(n) && old_n_count+1==n_count));
		Contract.Invariant(() -> ordered());
		return true;
	}
	...
	public boolean delete(int n) throws Exception {
		...
		Contract.Ensures(() -> (member(n) && old_n_count-1==n_count)||(!member(n) && old_n_count==1));
		Contract.Invariant(() -> ordered());
		return b;
	}
...
class AbsTree_Iterator {
	...
	public int next() throws Exception {
		...
		count--;
		Contract.Ensures(() -> value > old_value);
		return value;
	}
	...
	private void stack_left_spine(AbsTree node) throws Exception {
		...
		Contract.Ensures(() -> (node2==null || (stack.peek().value == node2.min().value)&&(value<node2.value)));
	}
